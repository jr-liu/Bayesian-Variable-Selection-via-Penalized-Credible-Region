...
To be more specific, for the proposed approach, only the parameters in the full model
are assigned prior distributions. Thus, priors are specified only for $\beta$ and $\sigma^2$
(usually simple conjugate priors), with
\begin{equation}
\beta \sim \mathcal{N}\!\left(0, \frac{\sigma^2}{\tau} I_p \right),
\qquad
\tau \sim \mathrm{Gamma}(\alpha, \beta).
\end{equation}

The quantity $\lVert \beta \rVert_0$ is defined as the $\ell_0$ norm of the vector, which equals
the number of nonzero elements in $\beta$. The sparsest model is obtained as
\begin{equation}
\tilde{\beta}
=
\arg\min_{\beta} \lVert \beta \rVert_0
\quad \text{subject to} \quad
\beta \in C_\alpha.
\end{equation}
That is, $\tilde{\beta}$ is the element of $C_\alpha$ with the minimum number of nonzero
components. The selected model at a given $\alpha$ is then denoted by
\begin{equation}
A_n^\alpha = \{ j : \tilde{\beta}_j \neq 0 \}.
\end{equation}

If $\tau$ is fixed, the joint credible region for $\beta$ is elliptical and takes the form
\begin{equation}
C_\alpha
=
\left\{
...
\right\},
\end{equation}
where $\hat{\beta}$ and $\Sigma$ denote the posterior mean of $\beta$ and the posterior
covariance matrix, respectively, both of which admit closed-form expressions. If $\tau$
is instead assigned a prior distribution, a short MCMC run is required to obtain
$\hat{\beta}$ and $\Sigma$. Although the resulting credible region is no longer elliptical,
